...
svmPoly_grid <- grid_regular(degree(),
                             cost(),
                             levels = 10)
svmRadial_grid <- grid_regular(rbg_sigma(),
                               cost(),
                               levels = 10)
...
svm_folds <- vfold_cv(AEAC_Train, v = 10, repeats=1)
...
vroom_write(x=svmPoly_preds, file="SVM_Poly.csv", delim=",") #"Amazon_AEAC_Kaggle/KNN.csv"
